...
### ADR-020: Fundamentals layer exposes raw posterior coefficients, not predicted shares
- **Status**: accepted (2026-05-22)
- **Context**: Issue #31 ships the presidential fundamentals regression (`pipeline/models/fundamentals.py`). The downstream combiner (#32) blends the fundamentals signal with the Linzer/Kremp poll aggregator (#30) into a single first-round share posterior. Two interface shapes were considered: (a) the fundamentals layer outputs a `FundamentalsPosterior` analogous to `AggregatorPosterior` — `(n_samples, n_candidates)` of predicted shares — so the combiner treats it as a prior over the same quantity, or (b) the fundamentals layer outputs raw per-feature coefficient samples plus the training-set standardisation parameters, and the combiner applies them to each candidate's feature vector itself.
- **Decision**: Output **raw regression coefficients**. `FundamentalsCoefficients` carries `samples: dict[name → ndarray]` keyed by feature name (one 1-D array of `n_samples` posterior draws per coefficient), plus `intercept_samples`, `sigma_samples`, `feature_means`, `feature_sds`, and diagnostics. A `predict()` helper applies the standardisation and the linear predictor + optional sigma noise to produce posterior predictive shares (used by the in-module backtest and available to the combiner if it wants the simple path). The combiner is free to apply the coefficients its own way — e.g., weighted blend on the logit scale rather than on share scale.
- **Consequences**: The fundamentals layer is decoupled from candidate identity — the same coefficient posterior generalises to any candidate whose 8-feature vector is constructable. The combiner owns the candidate × feature matrix at forecast time and can use that flexibility (e.g., per-candidate sentiment differential that wasn't present in any historical row). Backtest coverage is computed inside the fundamentals module by calling `predict()` on held-out candidates; the issue #31 acceptance threshold (≥80% / ≥95%) tests the deterministic combination of (β posterior + σ noise) and is therefore a calibration check of the regression itself, independent of how #32 ends up weighting it.
